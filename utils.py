...
def display_hh_summary():
    ...
    with st.expander("Remarks"):
        ...
        "temperatures (as well as warming) to assess the effect of temperature on various features of the HH model dynamics. Note that the spike width " \
        "is defined as the mean spike width over all the action potentials or spikes generated by the neuron. The ISIs are defined as the time interval " \
        "between successive spikes, so are calculated for the case when there are more than 1 spikes generated by the neuron. ")
